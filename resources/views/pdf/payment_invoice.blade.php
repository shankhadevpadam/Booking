		...
		<div class="invoice-container">
            ...
            <div class="row invoice-address">
                ...
                <div class="col invoice-address_right text-right">
                    <p>BILL TO</p>
                    <p><strong>{{ Arr::get($user_package, 'user.name') }}</strong><br>{{ Arr::get($user_package, 'user.country.name') }}<br> Email: {{ Arr::get($user_package, 'user.email') }}</p>
                </div>
            </div>

            <div class="invoice-number">
                <h3>Invoice: {{ strtotime($created_at) }}</h3>
                <div class="text-left">
                    <small>Invoice Date: {{ Carbon\Carbon::parse($created_at)->format('F d, Y') }}</small>
                </div>
            ...
            <table class="table invoice-table">
                ...
                <tbody>
                    <tr>
                        <td>
							<h4>{{ Arr::get($user_package, 'package.name') }}</h4>
						</td>
						<td>{{ $user_package['number_of_trekkers'] }}</td>
                ...
                <tbody>
                    @if ($bank_charge > 0)
                        <tr>
                            ...
                            <td align="right">{{ number_format($bank_charge, 2) }}</td>
                        </tr>
                    @endif
                    <tr>
                        <td colspan="2" class="text-right">
                            <h4>Total</h4>
                            <small>Payment on {{ Carbon\Carbon::parse($created_at)->format('d F') }} using {{ Str::headline($payment_method) }}</small>
                        </td>
                        <td class="last text-right">
                            <h4>{{ number_format($amount + $bank_charge, 2) }}</h4>
                        </td>
                    </tr>
                </tbody>
            </table>
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    ...
                                    <span class="highlight">{{ $user_package['start_date'] }}</span>
                                </td>

                                <td>
                                    <strong>Trek End Date:</strong><br>
                                    <span class="highlight">{{ $user_package['end_date'] }}</span>
                                </td>
                            </tr>
                            <tr>
